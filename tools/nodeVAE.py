...
from tools.gnnModules import GatedConv1d, GatedConvTranspose1d
...
class Decoder(nn.Module):
    def __init__(self, nhid, numNodes, latent_space, satellites):
        ...
        self.decoder_XYZ = nn.ModuleList([
            ...
            #decode_layer(nhid * 4, nhid * 4, m), #nn.BatchNorm1d(nhid * 6),
            decode_layer(nhid * 4, nhid * 6, m), #nn.BatchNorm1d(nhid * 6),
            #decode_layer(nhid * 6, nhid * 6, m), #nn.BatchNorm1d(nhid * 6),
            decode_layer(nhid * 6, nhid * 8, m), #nn.BatchNorm1d(nhid * 6),
            #decode_layer(nhid * 8, nhid * 8, m), #nn.BatchNorm1d(nhid * 6),
            #decode_layer(nhid * 8, nhid * 10, m), #nn.BatchNorm1d(nhid * 6),
            #decode_layer(nhid * 10, nhid * 10, m), #nn.BatchNorm1d(nhid * 8),
            #decode_layer(nhid * 10, numNodes * 3, None)])
